# 👋 Hi, I'm Christopher

## 🚀 About Me
iOS engineer focused on SwiftUI, UIKit, and building polished consumer products. I’ve worked in startup environments shipping mobile features end-to-end across architecture, UI, backend integration, testing, and release.

My work spans early-stage product development, App Store launches, and performance-sensitive mobile features. I’m especially energized by products with strong UX, clear product thinking, and real ownership on the iOS side.

## 🔗 Connect With Me
- X / Twitter: [@chrisendress_io](https://twitter.com/chrisendress_io)
- LinkedIn: [christopher-endress-03bb8a291](https://www.linkedin.com/in/christopher-endress-03bb8a291)

## 📱 Selected Work
- **NoomaLooma** — shipped a creative wellness platform featured on the App Store. 
  [View on the App Store](https://apps.apple.com/us/app/noomalooma-creative-wellness/id6757317267)

- **Curato** — Best UI/UX winner at The Phia Hack, a swipe-based shopping experience built around vibe-driven discovery  
  [View on Devpost](https://devpost.com/software/curato-5lkcbt?_gl=1*17gvklc*_gcl_au*MjAwNzU5OTU0LjE3NzY2MTE1NTI.*_ga*Nzk4ODEwOTg3LjE3NzY2MTE1NTM.*_ga_0YHJK3Y10M*czE3NzY3OTk1NjUkbzUkZzEkdDE3NzY4MDY0NjIkajQ0JGwwJGgw)

- **AudioScribe** — offline iOS voice-notes app built with SwiftUI, SwiftData, and AVAudioEngine  
  [View on GitHub](https://github.com/cendress/AudioScribe)

- **iMirror: Reflect & Grow** — mindfulness and journaling app  
  [View on the App Store](https://apps.apple.com/us/app/imirror-reflect-grow/id6478330335)

- **Globe: World History** — education app with 12,000+ downloads  
  [Browse repositories](https://github.com/cendress?tab=repositories)

## 🌟 Highlights
- Built and shipped the iOS experience for **NoomaLooma**
- Rebuilt Sweep360’s iOS codebase, improving SonarQube quality from **D to A**
- Won **Best UI/UX** at **The Phia Hack** with **Curato**
- Won the **Swift24 Hackathon**
- Published multiple apps to the App Store, including **iMirror**, **CentSage**, and **Globe: World History**

## 💼 Open To
- Product-focused iOS roles
- Startup opportunities with strong ownership
- Select freelance / contract work

## 🤝 Reach Out
Feel free to reach out about iOS roles, collaborations, or interesting products.

- 📧 Email: [chrisendress9@gmail.com](mailto:chrisendress9@gmail.com)
- 📬 DM me on X: [@chrisendress_io](https://twitter.com/chrisendress_io)

---
⭐️ From [cendress](https://github.com/cendress)


